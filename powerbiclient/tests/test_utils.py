...
TABLE_SCHEMA_LIST = [{ 'name': 'dummy_table_name', 'columns': TABLE_SCHEMA_COLUMNS }]
DATA_ROWS = [[1]]
...
class TestIsDatasetCreateConfigValid:
    def test_happy_path(self):
        assert is_dataset_create_config_valid({'locale': LOCALE, 'tableSchemaList': TABLE_SCHEMA_LIST, 'data': DATA})
    ...
    def test_invalid_locale(self):
        ...
        assert not is_dataset_create_config_valid({'local': LOCALE, 'tableSchemaList': TABLE_SCHEMA_LIST, 'data': DATA})

        # locale is not a string
        assert not is_dataset_create_config_valid({'locale': 1, 'tableSchemaList': TABLE_SCHEMA_LIST, 'data': DATA})

    def test_invalid_table_schema_list(self):
        # tableSchemaList key does not exist
        assert not is_dataset_create_config_valid({'locale': LOCALE, 'tableSchem': TABLE_SCHEMA_LIST, 'data': DATA})

        # tableSchemaList is not a list
        assert not is_dataset_create_config_valid({'locale': LOCALE, 'tableSchemaList': 1, 'data': DATA})

        # tableSchemaList is empty list
        assert not is_dataset_create_config_valid({'locale': LOCALE, 'tableSchemaList': [], 'data': DATA})

        # table does not have name
        assert not is_dataset_create_config_valid({'locale': LOCALE, 'tableSchemaList': [{'columns': TABLE_SCHEMA_COLUMNS}], 'data': DATA})

        # table does not have columns
        assert not is_dataset_create_config_valid({'locale': LOCALE, 'tableSchemaList': [{'name': 'dummy_table_name'}], 'data': DATA})

    def test_invalid_data(self):
        # data key does not exist
        assert not is_dataset_create_config_valid({'locale': LOCALE, 'tableSchem': TABLE_SCHEMA_LIST, 'dta': DATA})

        # data is not a list
        assert not is_dataset_create_config_valid({'locale': LOCALE, 'tableSchemaList': TABLE_SCHEMA_LIST, 'data': 1})

        # data is empty list
        assert not is_dataset_create_config_valid({'locale': LOCALE, 'tableSchemaList': TABLE_SCHEMA_LIST, 'data': []})

        # table does not have name
        assert not is_dataset_create_config_valid({'locale': LOCALE, 'tableSchemaList': TABLE_SCHEMA_LIST, 'data': [{'rows': DATA_ROWS}]})

        # table does not have rows
        assert not is_dataset_create_config_valid({'locale': LOCALE, 'tableSchemaList': TABLE_SCHEMA_LIST, 'data': [{'name': 'dummy_table_name'}]})
...
class TestGetDatasetCreateConfig:
    ...
    def test_happy_path_get_dataset_config(self):
        ...
        columns_types = [col['dataType'] for col in dataset_create_config['tableSchemaList'][0]['columns']]
        expected_column_types = ['Text', 'Logical', 'Int32', 'Number', 'DateTime', 'DateTimeZone', 'Text']
        assert columns_types == expected_column_types

        # validate data rows
        rows = dataset_create_config['data'][0]['rows']
        expected_rows = [
            ['a', 'true', '1', '1.2', "2022-10-27 10:50:00", "2022-10-27 10:50:00-07:00", '1'], 
            ['b', 'false', '2', '-4.3', "2022-10-27 11:50:00", "2022-10-27 11:50:00-07:00", 'a']
        ]
    ...
    def test_locale(self):
        dataset_create_config = get_dataset_config(pd.DataFrame([1, 2, 3]), locale='he-IL')
